...
struct GameState {
    ...
    int animationFrame;
    bool justAte;  // Flag to track if player just ate something
};

// Function prototypes
void initializeGame(GameState& game);
void handleInput(GameState& game, sf::RenderWindow& window, sf::Sound& munchSound);
void updateGame(GameState& game, sf::Sound& munchSound, sf::Sound& deathSound, sf::Sound& powerSound);
bool isValidMove(const GameState& game, int x, int y);
void moveEnemies(GameState& game);
void updatePumps(GameState& game, sf::Sound& powerSound);
void drawGame(GameState& game, sf::RenderWindow& window, sf::Font& font, 
              sf::Sprite& playerSprite, sf::Sprite& enemySprite, 
              sf::Sprite& dirtSprite, sf::Sprite& rockSprite, 
              sf::Sprite& pumpSprite);
...
int main() {
    ...
    sf::RenderWindow window(sf::VideoMode(WINDOW_WIDTH, WINDOW_HEIGHT), "Dig Dug - SFML with Sprites and Music");
    ...
    // Load textures
    sf::Texture playerTexture, enemyTexture, dirtTexture, rockTexture, pumpTexture;
    
    if (!playerTexture.loadFromFile("/home/nika/digdug_game/sprites/player.png") ||
        !enemyTexture.loadFromFile("/home/nika/digdug_game/sprites/enemy.png") ||
        !dirtTexture.loadFromFile("/home/nika/digdug_game/sprites/dirt.png") ||
        !rockTexture.loadFromFile("/home/nika/digdug_game/sprites/rock.png") ||
        !pumpTexture.loadFromFile("/home/nika/digdug_game/sprites/pump.png")) {
    ...
    sf::Sprite pumpSprite(pumpTexture);
    
    // Load music - commented out to remove background music
    /*
    sf::Music music;
    if (!music.openFromFile("/home/nika/digdug_game/audio/soothing_music.wav")) {
        std::cerr << "Error loading music" << std::endl;
        // Continue without music
    } else {
        music.setLoop(true);  // Loop the music
        music.setVolume(40);  // Set volume to 40% (lower for more soothing effect)
        music.play();         // Start playing
    }
    */
    
    // Load sound effects
    sf::SoundBuffer munchBuffer, deathBuffer, powerBuffer;
    if (!munchBuffer.loadFromFile("/home/nika/digdug_game/audio/effects/munch.wav")) {
        std::cerr << "Error loading munch sound" << std::endl;
        // Continue without sound
    }
    
    if (!deathBuffer.loadFromFile("/home/nika/digdug_game/audio/effects/death.wav")) {
        std::cerr << "Error loading death sound" << std::endl;
        // Continue without sound
    }
    
    if (!powerBuffer.loadFromFile("/home/nika/digdug_game/audio/effects/power.wav")) {
        std::cerr << "Error loading power sound" << std::endl;
        // Continue without sound
    }
    
    sf::Sound munchSound(munchBuffer);
    munchSound.setVolume(60);  // Set volume to 60%
    
    sf::Sound deathSound(deathBuffer);
    deathSound.setVolume(70);  // Set volume to 70%
    
    sf::Sound powerSound(powerBuffer);
    powerSound.setVolume(65);  // Set volume to 65%
    ...
    while (window.isOpen() && !game.gameOver) {
        // Handle events
        handleInput(game, window, munchSound);
        
        // Update game state
        updateGame(game, munchSound, deathSound, powerSound);
        
        // Draw the game
        window.clear(sf::Color::Black);
        drawGame(game, window, font, playerSprite, enemySprite, dirtSprite, rockSprite, pumpSprite);
        window.display();
    }
    
    // Play death sound when game is over
    if (game.gameOver) {
        deathSound.play();
        sf::sleep(sf::seconds(1.5)); // Wait for death sound to finish
    }
    
    // Stop music when game is over - commented out since we removed background music
    // music.stop();
    ...
    while (window.pollEvent(event)) {
        ...
        if (event.type == sf::Event::KeyPressed) {
            ...
            if (isValidMove(game, newX, newY)) {
                ...
                // If moving into dirt, increase score and play munch sound
...
void updateGame(GameState& game, sf::Sound& munchSound, sf::Sound& deathSound, sf::Sound& powerSound) {
    ...
    if (game.animationClock.getElapsedTime().asSeconds() >= 0.2) {
        game.animationFrame = (game.animationFrame + 1) % 2; // Toggle between 0 and 1
        game.animationClock.restart();
        
        // Reset the justAte flag
        game.justAte = false;
    }
    
    // Update pumps
    updatePumps(game, powerSound);
    ...
    for (auto it = game.pumps.begin(); it != game.pumps.end(); ) {
        ...
        for (auto enemyIt = game.enemies.begin(); enemyIt != game.enemies.end(); ) {
            if (enemyIt->first == it->first && enemyIt->second == it->second) {
                ...
                // Play power sound when enemy is defeated
                powerSound.play();
            } else {
                ++enemyIt;
            }
...
void drawGame(GameState& game, sf::RenderWindow& window, sf::Font& font, 
    ...
    for (int i = 0; i < HEIGHT; i++) {
        for (int j = 0; j < WIDTH; j++) {
            switch (game.map[i][j]) {
                case '#': // Dirt
                    dirtSprite.setPosition(j * TILE_SIZE, i * TILE_SIZE);
                    window.draw(dirtSprite);
                    break;
                case 'O': // Rock
                    rockSprite.setPosition(j * TILE_SIZE, i * TILE_SIZE);
                    window.draw(rockSprite);
                    break;
                case '*': // Pump
                    pumpSprite.setPosition(j * TILE_SIZE, i * TILE_SIZE);
                    window.draw(pumpSprite);
                    break;
                default:
                    break;
            }
    ...
    // Draw enemies
    for (const auto& enemy : game.enemies) {
        enemySprite.setPosition(enemy.first * TILE_SIZE, enemy.second * TILE_SIZE);
        window.draw(enemySprite);
    }
    
    // Draw player with rotation based on direction
    playerSprite.setPosition(game.playerX * TILE_SIZE, game.playerY * TILE_SIZE);
    
    // Set the origin to the center for proper rotation
    playerSprite.setOrigin(TILE_SIZE / 2, TILE_SIZE / 2);
    playerSprite.setPosition(
        game.playerX * TILE_SIZE + TILE_SIZE / 2, 
        game.playerY * TILE_SIZE + TILE_SIZE / 2
    );
